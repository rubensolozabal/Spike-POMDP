...
class Mlp(PyTorchModule):
    def __init__(
        ...
        self.hidden_sizes = hidden_sizes
        self.hidden_activation = hidden_activation
        self.hidden_activation = [snn.Leaky(beta=beta, spike_grad=spike_grad), snn.Leaky(beta=beta, spike_grad=spike_grad)]
        self.output_activation = output_activation
        self.output_activation = snn.Leaky(beta=beta, spike_grad=spike_grad)
        self.layer_norm = layer_norm
        self.fcs = []
        self.layer_norms = []
        in_size = input_size
        ...
        if len(input.shape) == 2:
            ...
            for t in range(T):
                ...
                for i, fc in enumerate(self.fcs):
                    ...
                    h, mem = self.hidden_activation[i](h, hidden_state[i].cuda())
                    hidden_state[i] = mem

                preactivation = self.last_fc(h)
                h, mem = self.output_activation(preactivation)

                spk_rec.append(h)

            # Stack and sum 
            output = torch.stack(spk_rec, dim=0)
            ...
            hidden_state = [act.init_leaky() for act in self.hidden_activation]
